...
uygulama.bindForms = function(){
      ...
      document.querySelector("form").addEventListener("submit", function(e){
        ...
        uygulama.istemci.istek(undefined,path,method,undefined,payload,function(statusCode,responsePayload){
          ...
          if(statusCode !== 200){
            ...
            var error = typeof(responsePayload.bilgi) == 'string' ? responsePayload.bilgi : 'Bilinmeyen bir hata oluştu, lütfen tekrar deneyin :(';
  ...
  uygulama.formResponseProcessor = function(formId,requestPayload,responsePayload){
    ...
    if(formId == 'hesapOluştur'){
      ...
      uygulama.istemci.istek(undefined,'api/belirtecler','POST',undefined,newPayload,function(newStatusCode,newResponsePayload){
        // Display an error on the form if needed
        if(newStatusCode !== 200){
  
          // Set the formError field with the error text
          document.querySelector("#"+formId+" .formError").innerHTML = 'Üzgünüm, bir hata oluştu :( Lütfen tekrar deneyin';
  ...
  uygulama.renewToken = function(callback){
    var currentToken = typeof(uygulama.yapılandırma.oturumBelirteci) == 'object' ? uygulama.yapılandırma.oturumBelirteci : false;
    if(currentToken){
      ...
      uygulama.istemci.istek(undefined,'api/belirtecler','PUT',undefined,payload,function(statusCode,responsePayload){
        // Display an error on the form if needed
        if(statusCode == 200){
          // Get the new token details
          var queryStringObject = {'kimlik' : currentToken.id};
          uygulama.istemci.istek(undefined,'api/belirtecler','GET',queryStringObject,undefined,function(statusCode,responsePayload){
            // Display an error on the form if needed
            if(statusCode == 200){
              uygulama.setSessionToken(responsePayload);
              callback(false);
